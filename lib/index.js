...
const fetchEntities = require('./wikidata')
...
const defaults = {
	logging: false,
	wikidata: false // fetch information for wikidata entries
}
...
const osmTransitLines = async (bbox, opt = {}) => {
	...
	if (options.wikidata) {
		const wikidataIds = uniq(lines.map(line => line.wikidata).filter(x => !!x))
		const wikidataEntities = await fetchEntities(wikidataIds)
		lines.forEach(line => {
			if (line.wikidata) {
				const entity = wikidataEntities[line.wikidata]
				line.wikidataClaims = entity ? entity.claims : null
			}
		})
	}
